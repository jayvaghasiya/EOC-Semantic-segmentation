...
parser.add_argument("--destination", type=str, help="path for the augmented dataset")
...
for index, file in enumerate(files):
    if file.endswith(".jpg"):
        ...
        shutil.copy(
            f"{annotation_path}", f'{augmentedPath}/{file.replace("jpg", "png")}'
        )
        ...
        for i in range(1):
            try:
                aug_count += 1
                imageName = img_path.split("/")[-1].split(".")[0] + "_{}".format(
                    aug_count
                )
                flip_horizontal(
                    in_img,
                ...
                for i in range(20):
                    try:
                        aug_count += 1
                        imageName = img_path.split("/")[-1].split(".")[
                            0
                        ] + "_{}".format(aug_count)
                        # print(7, i)
                        RandomCrop(
